...
# Exploring Data
#The data file on the course site returned an error saying unauthorized 
#so I could not download the original
...
#A Chi-Square test asks if the proportions of one variable (BMI) within groups of another variable (age) stray from the null model of equal proportions ◦The mosaic plot is a visual representation of this
...
#Generalizations
  ...
  # geom_text for ages (i.e. the x axis)
  p1 %+% DF_all + 
    geom_text(aes(x = max(xmax), label = X),
              y = 1, angle = 90,
              size = 3, hjust = 1,
              show.legend = FALSE)
